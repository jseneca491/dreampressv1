...
class CssParser extends \JchOptimize\Minify\Base
{
	...
	public function optimizeCssDelivery(&$sCombinedCss, $sHtml)
	{
		...
		##<procode>##
		$aDynamicSelectors = Helper::getArray($this->params->get('pro_dynamic_selectors', array()));

		if ($this->params->get('pro_remove_unused_css', '0') && !empty($aDynamicSelectors))
		{
			$sDynamicCss = $this->getDynamicCss($aDynamicSelectors, $sUnusedCss);

			if ($this->params->get('debug', '0'))
			{
				$sCombinedCss .= $this->sLnEnd . $this->sLnEnd . $this->sLnEnd . '/***!  Dynamic Css  !***/' .
					$this->sLnEnd . $this->sLnEnd;
			}

			$sCombinedCss .= $sDynamicCss;
		}
		##</procode>##
	...
	/**
	 * Function to get CSS used to render dynamic content when 'remove unused css' is enabled
	 *
	 * @param   array   $aDynamicSelectors  array of css selectors from 'pro_dynamic_selectors' to search in unused css for
	 * @param   string  $sUnusedCss         Css removed from the combined CSS file evaluated as not used on the page
	 *
	 * @return   string   portion of unused CSS that may be used by $aDynamicSelectors
	 */
	public function getDynamicCss($aDynamicSelectors, $sUnusedCss)
	{
		//Evaluate each css rule in unused css
		$sDynamicCss = preg_replace_callback('#' . self::cssRulesRegex() . '#', function ($aMs) use ($aDynamicSelectors) {
			//Recurse into each conditional group rule
			if (preg_match('#^@(?:media|supports|document)#', ltrim($aMs[0])))
			{
				$sMedCss = $this->getDynamicCss($aDynamicSelectors, $aMs[4]);

				if ($sMedCss)
				{
					return $aMs[2] . '{' . $sMedCss . '}' . $this->sLnEnd;
				}
			}

			foreach ($aDynamicSelectors as $sDynamicSelector)
			{
				if (strpos($aMs[1], $sDynamicSelector) !== false)
				{
					return $aMs[0];
				}
			}
	...
	public function extractCriticalCss($aMatches, $oXPath, $sHtmlAboveFold, $sFullHtml, &$sCriticalCss, &$sUnusedCss)
	{
		...
		if (empty($aFoundSelectorChains))
		{
			...
			##<procode>##
			//continue checking the full HTML if we want to remove unused css
			if ($this->params->get('pro_remove_unused_css', '0'))
			{
				foreach ($aSelectorChains as $sSelectorChain)
				{
					if ($this->checkCssAgainstHtml($sSelectorChain, $sFullHtml))
					{
						return $aMatches[0];
					}
				}

				//Not found! remove from combined CSS
				$sUnusedCss .= $aMatches[0];
